...
(defun ascii-list-to-floating (ascii-list)
    (let ((to-integer (ascii-list-to-integer ascii-list)))
        (float (* to-integer (expt 10 (* -1 (length ascii-list)))))))
...
(defun parse-boolean-job (json-list record value)
    (let ((boolean-rest
        (parse-boolean json-list (string-to-ascii-list (first record)))))
        (if (not (null boolean-rest))
            (cons (first value) (rest boolean-rest))
            (if (and (null (rest record)) (null (rest value)))
                NIL
                (parse-boolean-job json-list (rest record) (rest value))
            )
        )))
                            ...
                            ((eql (second (rest base-rest)) plus-sign)
                                (let ((exp-rest (digits (cdddr base-rest))))
                                    (cons
                                        ...
                                        (rest exp-rest))))
                            ((eql (second (rest base-rest)) minus-sign)
                                (let ((exp-rest (digits (cdddr base-rest))))
                                    (cons
                                        (calc-exponential 
                                            (first base-rest) 
                                            (* -1 
                                                (ascii-list-to-integer (first exp-rest))))
                                        (rest exp-rest))))
                            ((not (is-sign (second (rest base-rest))))
            ...
            (let ((floating-rest (digits (cddr integer-rest))))
                (let ((floating-part
                    ...
                    (if (>= (first integer-rest) 0)
                        (cons (+ 
                            (first integer-rest) floating-part) 
                            (rest floating-rest))
                        (cons (- 
                            (first integer-rest) floating-part) 
                            (rest floating-rest))
...
(defun parse-my-integer (json-list)
    (if (null json-list)
        ...
        (if (eql (first json-list) minus-sign)
            (let ((number-rest (digit (rest json-list))))
...
(defun digit (json-list)
    (if (eql (first json-list) zero)
        (cons (list (first json-list)) (rest json-list))
...
(defun json-string (json-list)
    (if (and (eql (first json-list) dbl-quotes) 
        ...
        (let ((string-rest (parse-string (rest json-list)))) 
            (cons (ascii-list-to-string (first string-rest)) 
                (rest string-rest)))
        NIL
    ))
...
(defun parse-string (json-list)
    (cond ((eql (first json-list) dbl-quotes)
        ...
        (T (if (eql (first json-list) (char-code #\\))
            (if (not (null (parse-escape (rest json-list))))
                (let ((ret-value (parse-string (cddr json-list))))
                    (cons
                        (cons (first json-list) 
                            (cons (second json-list) 
                                (first ret-value)))
                        (rest ret-value)))
                (if (not (null (parse-escape-u (rest json-list))))
                    (let ((ret-value (parse-string (cddr (cddddr json-list)))))
                        (cons
                            (cons (first json-list) 
                                (cons (second json-list) 
                                    (cons (third json-list) 
                                        (cons (fourth json-list) 
                                            (cons (fifth json-list) 
                                                (cons (sixth json-list) 
                                                    (first ret-value)))))))
                            (rest ret-value)))
                    (call-error)
                )
        ...
        (let ((trim-rest (trim-head (rest attributes-rest))))
            (if (not (null trim-rest))
                (if (eql (first trim-rest) colon)
                    (let ((value-rest (json-value (trim-head (rest trim-rest)))))
                        (if (not (null value-rest))
                            (cons 
                                (list (first attributes-rest) (first value-rest))
        ...
        (let ((trim-rest (trim-head (rest value-rest))))
            (if (not (null trim-rest))
                (if (eql (first trim-rest) comma)
                    (let ((elements-rest (json-elements (trim-head (rest trim-rest)))))
                        (cons (append (list (first value-rest)) (first elements-rest)) (rest elements-rest)))
...
(defun json-members (json-list)
    (let ((pair-rest (json-pair (trim-head json-list))))
        (let ((trim-rest (trim-head (rest pair-rest))))
            (if (not (null trim-rest))
                (if (eql (first trim-rest) comma)
                    (let ((members-rest (json-members (trim-head (rest trim-rest)))))
                        (cons (append (list (first pair-rest)) (first members-rest)) (rest members-rest)))
            ...
            (let ((after-bracket (trim-head (rest trim-json-list)))) 
                (if (not (null after-bracket))
                    (if (eql (first after-bracket) cls-sqr-bracket)
                        ...
                        (let ((elements-rest (json-elements (trim-head after-bracket))))
                            (if (eql (first (rest elements-rest)) cls-sqr-bracket)
                                (cons (append (list json-array-value) (first elements-rest)) (trim-head (cddr elements-rest)))
                                (call-error)
                            )))
                    (call-error)))
            NIL)))
            ...
            (let ((after-brace (trim-head (rest trim-json-list))))
                (if (not (null after-brace))
                    (if (eql (first after-brace) cls-brace)
                        ...
                        (let ((members-rest (json-members (trim-head after-brace))))
                            (if (eql (first (rest members-rest)) cls-brace)
                                (cons (append (list json-obj-value) (first members-rest)) (trim-head (cddr members-rest)))
                                (call-error)
                            )))
                    (call-error)))
            NIL)))
...
(defun jsonparse (json-string)
    (if (stringp json-string)
        (let ((json-rest (json-object (trim-head (string-to-ascii-list json-string)))))
        ...
        (if (null fields)
            ...
            (cond ((eql (first json) json-obj-value)
                    (if (stringp (first fields))
                        (let ((field (pair-finder (rest json) (first fields))))
                            (if (not (null field))
...
(defun json-object-write (json)
    (if (listp json)
        (cond ((equal json-obj-value (first json))
                (concatenate 'string "{ " (json-obj-write (rest json)) " }"))
            ((equal json-array-value (first json))
                (concatenate 'string "[ " (json-array-write (rest json)) " ]"))
            (T (call-error)))
        (call-error)
    ))
...
(defun json-members-write (json-members)
    (cond ((null (rest json-members)) 
        ...
        (T (concatenate 'string (json-pair-write (first json-members)) 
            ", "
...
(defun json-elements-write (json-elements)
    (cond ((null (rest json-elements)) 
        ...
        (T (concatenate 'string (json-value-write (first json-elements)) 
            ", "
